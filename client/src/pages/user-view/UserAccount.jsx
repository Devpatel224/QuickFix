...
import AddReview from "@/components/user-view/review/AddReview";
...
const getRequestMessage = (req) => {
  ...
  if (requestStatus === "pending") {
    return (
      <p className="text-yellow-600 font-medium mt-2">
        Your request is <span className="underline">pending</span>. The provider
  ...
  if (requestStatus === "declined") {
    return (
      <div className="text-red-600 font-medium mt-2">
        <p>
          Your request was <span className="underline">declined</span> by{" "}
          {provider?.name}.
        </p>
        {providerNote && (
  ...
  if (requestStatus === "accepted") {
    ...
    if (workStatus === "pending") {
      message =
        "Your request has been accepted. The provider will deliver the service on the requested date.";
      ...
      <div className="text-green-700 mt-2">
        ...
        {providerNote && (
          <div className="mt-2 bg-green-100 p-3 rounded-md border-l-4 border-green-500">
            <p className="text-gray-800">
              <span className="font-bold text-green-600">Provider's Note:</span>{" "}
              {providerNote}
            </p>
          </div>
        )}
...
function UserAccount() {
  ...
  let filterredRequests = requests?.filter(
    (req) =>
      req.requestStatus !== "declined" &&
      (req.requestStatus === "pending" ||
        req.workStatus === "pending" ||
        req.workStatus === "in progress"),
  );
  let completedRequests = requests?.filter(
    (req) => req.workStatus === "completed" || req.requestStatus === "declined",
  );
  ...
  return (
    ...
    >
      ...
      <h3 className="text-lg font-semibold mt-6 self-start max-w-5xl w-full">
        Service Requests
      </h3>
              ...
              <Card key={req._id}>
                <CardContent className="p-4">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    ...
                    <span className="text-blue-600">WorkStatus</span>:{" "}
                    {getWorkStatusBadge(req.workStatus)}
                  </div>
